...
def translation(firstAccount: BankAccount, secondAccount: BankAccount, money: int):
    if firstAccount.account == secondAccount.account:
        if firstAccount.balance < money:
            print('error 404!')
            print('-'*43) 
        else:
            firstAccount.balance -= money
            secondAccount.balance += money
            print('Translation successfully completed')
            print('-'*43)
    else:
        firstAccount.moneyConversion(secondAccount.account)
        if firstAccount.balance < money:
            print('error 404!')
            print('-'*43) 
        else:
...
while True:
    ...
    if command == 0:
        ...
        print('if u want to tranlate money someone type "translation" and number your account and number second account and money ')
        print('-'*43)
        cmd = input().split()
        if cmd[0] == 'balance':
            ...
            translation(db[int(cmd[1])-1], db[int(cmd[2])-1], int(cmd[3]))                     
